            ...
            <div className="flex flex-wrap gap-2 md:gap-4 w-full">
              {siteData.pastEvents.map((event) => (
                <button
                  ...
                  className={`relative px-4 py-2 md:px-8 md:py-4 text-sm md:text-2xl font-black tracking-tighter transition-all rounded-xl md:rounded-2xl overflow-hidden group ${activeYear === event.year ? "text-white bg-ted-red shadow-2xl" : "text-white/30 hover:text-white hover:bg-white/5"
                    }`}
                >
                  {event.year}
          ...
          <div className="order-2 lg:order-1 flex flex-col items-start space-y-10">
            <div>
              <motion.h3
                ...
                className="text-[10vw] sm:text-5xl md:text-7xl font-black text-white uppercase tracking-tighter mb-4 leading-[0.9] text-balance"
              >
                TEDx <span className="text-ted-red">VNRVJIET</span> <br />
                <span className="text-white/40 italic text-balance">{activeEvent.theme}</span>
              </motion.h3>
            ...
            <Link
              href={`/archive/${activeEvent.slug}`}
              className="group flex items-center space-x-4 px-10 py-5 bg-white text-black font-black text-sm tracking-[0.2em] rounded-full shadow-[0_0_30px_rgba(255,255,255,0.1)] transition-all hover:bg-ted-red hover:text-white hover:scale-105"
            >
